...
def loadModel(modelPath):
    ...
    i3d.load_state_dict(torch.load(modelPath))
    i3d.cuda()
    i3d.eval()
    return i3d

def extractFeatures(model,input,saveDir,saveName):
    with torch.no_grad():
        input = input.cuda()
...
if __name__ == '__main__':
    modelPath="/root/Pycharm_Project/pytorch-i3d/model/flownet_imagenet.pt"
    saveRoot="/mnt/MSR-VTT/MSR-VTT-i3dfeatures/"
    videosRoot="/mnt/MSR-VTT/MSR-VTT/"

    import numpy as np
    import cv2

    cap = cv2.VideoCapture('/mnt/MSR-VTT/MSR-VTT/video7003.mp4')
    ...
    i3dModel=loadModel(modelPath)
    videoFiles = os.listdir(videosRoot)
    for videoFile in tqdm(videoFiles):
        videoPath = os.path.join(videosRoot, videoFile)
        try:
            videoData = extractVideoFrames(videoPath, 80)
            npyFile = videoFile.replace(".mp4", "_flow_.npy")
            extractFeatures(i3dModel, videoData, saveRoot, npyFile)
        except:
            print("bad video {} ".format(videoFile))
